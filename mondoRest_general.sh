...
if [[ -f ${outDir}/epiPrepped_blur6mm.nii.gz ]];then
	echo ""
	echo "!!!!!!!!!!!!!!!!Mondo Rest has already bee processed for this Dir!!!!!!!!!!!!!!!!!!"
...
if [[ $lenC == 0 ]];then
	3dTproject -input ${outDir}/epiWarped.nii.gz -mask ${templateDir}/${templatePre}BrainExtractionMask_2mmDil1.nii.gz  -prefix ${tmpOutDir}/epiPrepped_blur6mm.nii.gz -ort ${outDir}/motion.1D -ort ${outDir}/motion_deriv.1D -ort ${outDir}/stim_regressors.txt -ort ${tmpOutDir}/pcRest.wm.csf_vec.1D -polort 1 -bandpass 0.008 0.10 -blur 6
##comments: Decided again a more restricted blur in mask with different compartments for cerebellum etc, because that approach seemed to be slighly harming tSNR actually and did not help with peak voxel or extent analyses when applied to Faces contrast. Decided to use a dilated Brain Extraction mask because this at least gets rid of crap that is way outside of brain. This saves space (slightly) and aids with cleaner visualizations. A GM mask can still later be applied for group analyses, this way we at least leave that up to the user.
	...
	3dTproject -input ${outDir}/epiWarped.nii.gz -mask ${templateDir}/${templatePre}BrainExtractionMask_2mmDil1.nii.gz -prefix ${tmpOutDir}/epiPrepped_blur6mm.nii.gz -CENSORTR $clist -ort ${outDir}/motion.1D -ort ${outDir}/motion_deriv.1D -ort ${outDir}/stim_regressors.txt -ort ${tmpOutDir}/pcRest.wm.csf_vec.1D -polort 1 -bandpass 0.008 0.10 -blur 6
